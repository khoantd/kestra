...
1. **Read `.agent/SESSION.md` first** — mandatory before planning or coding; use **`/resume`** to continue prior work
2. **Follow the workflow** — Use `/spec` → `/plan` → `/build` → `/review`
...
8. **Use the right agent** — Invoke specialized agents for their domains (personas also require SESSION)
