...
1. **Read `.agent/SESSION.md` first** — mandatory before planning or coding; run **`/resume`** when continuing prior work.
2. Follow the workflow and use the command prompts when starting a phase.
3. Apply **`.cursor/rules/`**; treat **`security.mdc`** as non-negotiable.
4. Prefer tests first and small, buildable changes.
5. **@ mention** the right **`.cursor/agents/`** file when the task matches that role (personas also require SESSION).
6. Update **`.agent/SESSION.md`** via **`/handoff`** before ending a session.
